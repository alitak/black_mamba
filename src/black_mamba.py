...
async def alacsonymodok(ctx, user: discord.User):
    ...
    debug(user.name + " " + guild.ally_codes[user.name])
    embed = discord.Embed(title="Alacsony modok " + user.name + " karakterein", color=0x00ffff)

    for mod in requests.get(black_mamba.swgoh_api["mods"].replace("%ALLYCODE%", guild.ally_codes[user.name])).json()["mods"]:
        if mod["rarity"] < 5:
...
async def twcompare(ctx, user: discord.User, ally2: int):
    ...
    if "status_code" in ember:
        ...
        await addWaitingReaction(ctx)
        seged = user.name
        ally1 = guild.ally_codes[user.name]
...
async def allycode(ctx):
    ...
    embed = discord.Embed()
    embed.add_field(name="Allycodeod: ", value=str(guild.ally_codes[seged]))
    await ctx.send(embed=embed)
    await addSuccessReaction(ctx)
